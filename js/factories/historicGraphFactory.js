...
define(['./module','moment'], function (factories,moment) {
    'use strict';
    factories.factory('historicGraphFactory', ['$rootScope','$state','$interval', '$http','$timeout', function ($rootScope,$state,$interval,$http,$timeout) {
		var historicTrafficData = [];
		$rootScope.pageWidth = window.innerWidth/2;
	    var historicGraphFactory = {
			...
			getIngressData: function(url,agg_name,vlanId,start_time,end_time,label,graph){
				...
				$http({
					...
					if($rootScope.counter === $rootScope.seriesLength){
						$rootScope.counter = 0;
